...
SELECT count( * ) AS "Number of free facilities"
FROM Facilities
WHERE membercost =0
...
FROM (
    SELECT DISTINCT Bookings.facid,
...
SELECT  Facilities.name AS "Facility",
		CASE WHEN Bookings.memid = 0 THEN 'Guest'
			ELSE CONCAT(Members.firstname,' ', Members.surname) END AS "Fullname",
...
AND CASE WHEN Bookings.memid = 0 THEN Facilities.guestcost*Bookings.slots >30
			ELSE Facilities.membercost*Bookings.slots >30 END
ORDER BY 3 DESC

/* Q9: This time, produce the same result as in Q8, but using a subquery. */
SELECT  Facilities.name as "Facilitiy",
		sub.Fullname,
		CASE WHEN sub.memid = 0 THEN Facilities.guestcost*sub.sub.slots
			ELSE Facilities.membercost*sub.slots END AS "Cost"
FROM (
	Select Bookings.facid AS facid,
...
AND CASE WHEN sub.memid = 0 THEN Facilities.guestcost*sub.slots >30
			ELSE Facilities.membercost*sub.slots >30 END
ORDER BY 3 DESC
